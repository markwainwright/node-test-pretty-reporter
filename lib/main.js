  ...
  for await (const event of source) {
    ...
    switch (type) {
      ...
      case 'test:plan':
        ...
        // file=undefined when all tests have completed
      ...
      case 'test:diagnostic':
        ...
        yield ''.padEnd((data.nesting + 1) * 2);
        const diagnosticMessage = `${format.logSymbols.info} ${data.message}`;
        yield format.chalk ? format.chalk.gray(diagnosticMessage) : diagnosticMessage;
        yield '\n';
        break;
      case 'test:stdout':
